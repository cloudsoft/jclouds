...
import org.jclouds.openstack.keystone.v3.handlers.RetryOnRenew;
import org.jclouds.openstack.keystone.v3.suppliers.LocationIdToURIFromTokenForTypeAndVersion;
import org.jclouds.openstack.keystone.v3.suppliers.RegionIdToAdminURIFromTokenForTypeAndVersion;
import org.jclouds.openstack.keystone.v3.suppliers.RegionIdToAdminURISupplier;
import org.jclouds.openstack.keystone.v3.suppliers.RegionIdToURIFromTokenForTypeAndVersion;
...
public class KeystoneAuthenticationModule extends AbstractModule {
   ...
   public static class ProviderModule extends AbstractModule {
      ...
      protected void configure() {
         install(new FactoryModuleBuilder().build(LocationIdToURIFromTokenForTypeAndVersion.Factory.class));
      }
      ...
      protected final Supplier<URI> provideZoneIdToURISupplierForApiVersion(
            ...
            LocationIdToURIFromTokenForTypeAndVersion.Factory factory) {
   ...
   public static class RegionModule extends AbstractModule {
      ...
      protected void configure() {
         install(new FactoryModuleBuilder().implement(RegionIdToURISupplier.class,
                 RegionIdToURIFromTokenForTypeAndVersion.class).build(RegionIdToURISupplier.Factory.class));
         install(new FactoryModuleBuilder().implement(RegionIdToAdminURISupplier.class,
               RegionIdToAdminURIFromTokenForTypeAndVersion.class).build(RegionIdToAdminURISupplier.Factory.class));
   ...
   @Singleton
   @Authentication
   protected final Supplier<String> provideAuthenticationTokenCache(final Supplier<Token> supplier)
   ...
   @Provides
   @Singleton
   protected final Map<String, Function<Credentials, Token>> provideAuthenticationMethods(Injector i) {
      return authenticationMethods(i);
   }

   protected Map<String, Function<Credentials, Token>> authenticationMethods(Injector i) {
      Builder<Function<Credentials, Token>> fns = ImmutableSet.<Function<Credentials, Token>> builder();
   ...
   @Provides
   @Singleton
   protected final Function<Credentials, Token> authenticationMethodForCredentialType(
   ...
   @Provides
   @Singleton
   public final LoadingCache<Credentials, Token> provideTokenCache(Function<Credentials, Token> getToken) {
   ...
   @Provides
   @Singleton
   protected final Supplier<Token> provideTokenSupplier(final LoadingCache<Credentials, Token> cache,
