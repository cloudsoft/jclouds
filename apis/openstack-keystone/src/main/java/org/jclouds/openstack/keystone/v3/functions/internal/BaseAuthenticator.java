 ...
 */
package org.jclouds.openstack.keystone.v3.functions.internal;
...
public abstract class BaseAuthenticator<C> implements Function<Credentials, Token> {
   ...
   @Override
   public Token apply(Credentials input) {
      ...
      if (tenantId.isPresent()) {
         token = authenticateWithTenantId(tenantId, creds);
      } else if (tenantName.isPresent()) {
         token = authenticateWithTenantName(tenantName, creds);
   ...
   protected abstract Token authenticateWithTenantId(Optional<String> tenantId, C apiAccessKeyCredentials);

   protected abstract Token authenticateWithTenantName(Optional<String> tenantId, C apiAccessKeyCredentials);
