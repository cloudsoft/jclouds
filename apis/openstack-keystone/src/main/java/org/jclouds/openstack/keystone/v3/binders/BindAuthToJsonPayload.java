...
public class BindAuthToJsonPayload extends BindToJsonPayload implements MapBinder {
   ...
   protected void addCredentialsInArgsOrNull(GeneratedHttpRequest gRequest, String templateName, Builder<String, Object> builder) {
...
//            builder.put(arg.getClass().getAnnotation(CredentialType.class).value(), arg);
            if (arg instanceof PasswordCredentials) {
               builder.put("identity", ImmutableMap.of("methods", ImmutableList.of("password"),
                       "password", ImmutableMap.of("user", ImmutableMap.of("name", ((PasswordCredentials) arg).getUsername(), "password", ((PasswordCredentials) arg).getPassword(), "domain", ImmutableMap.of("name", templateName))
               )));
            }
         }
      }
   ...
   public <R extends HttpRequest> R bindToRequest(R request, Map<String, Object> postParams) {
      ...
      addCredentialsInArgsOrNull(gRequest, (String) postParams.get("tenantName"), builder);
